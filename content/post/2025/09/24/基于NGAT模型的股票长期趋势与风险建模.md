...
**2. 训练设置**模型的所有超参数均通过在验证集上进行网格搜索确定，例如关系图构建的记忆窗口 $\delta$（最优为5天），预测周期 T（1天、5天、10天、21天，分别对应日、周、双周、月），以及注意力头数等。优化器采用Adam。

**3. 基线模型**为了全面评估NGAT的性能，本文选取了多种非图模型和图模型作为基线进行对比，包括：
