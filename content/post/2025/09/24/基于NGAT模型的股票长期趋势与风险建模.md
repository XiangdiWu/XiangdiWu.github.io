...
**4.1 序列嵌入 (Sequential Embedding)**

为捕捉股价自身的时间序列依赖性，模型首先使用循环神经网络（RNN）对历史交易数据进行编码。遵循业界常规做法，本文选用长短期记忆网络（LSTM）作为序列嵌入模块。

-   对于任意股票 s，在第 d 日，模型输入一个包含过去  天交易特征的时间序列矩阵 。
-   通过LSTM网络生成该股票的序列嵌入向量 ，并取最后一个时间步的输出作为最终的序列表示。

**4.2 关系嵌入 (Relational Embedding)**
...
-   -   模型中的企业关系边是基于公司在财经新闻或社交媒体（如推文）中的共现关系动态构建的。
    ...
    -   通过对每日新闻进行聚合，得到一个无条件的日度关系属性 。
    -   为了模拟新闻影响的持续性与时效性，模型引入了一个带遗忘机制的时间加权聚合方法，用于计算最终的**条件关系属性**，该方法给予近期的新闻更高的权重。
-   **节点级图注意力层 (Node-level Graph Attention Layer)** 这是NGAT模型的**核心创新**。与标准GAT为所有节点使用一套共享的注意力参数不同，NGAT为每个节点（公司）都分配了一套独立的注意力机制参数。这使得模型能够针对每家公司的独特性，学习个性化的信息聚合方式。
-   -   **注意力系数计算**: 对于节点i和它的邻居节点j，其注意力系数  的计算过程如下：
...
-   1.  首先，对节点的序列嵌入  和  进行线性变换，得到查询向量（Query）和键向量（Key）。
    1.  然后，将这两个向量拼接后，通过节点i专属的注意力参数  进行计算，并结合前述的条件关系属性  进行加权。
    1.  最终，使用LeakyReLU激活函数和Softmax进行归一化，得到最终的注意力系数。 其完整公式为：

**4.3 输出层 (Output Layer)**

最后，将经过多层关系嵌入模块处理后的最终表示向量  输入到一个全连接输出层，得到预测结果 。
...
**5.2 训练设置**模型的所有超参数均通过在验证集上进行网格搜索确定，例如关系图构建的记忆窗口 （最优为5天），预测周期 T（1天、5天、10天、21天，分别对应日、周、双周、月），以及注意力头数等。优化器采用Adam。

**5.3 基线模型**为了全面评估NGAT的性能，本文选取了多种非图模型和图模型作为基线进行对比，包括：
...
**1. 收益率趋势预测性能**实验结果明确显示，**NGAT在两个数据集上的收益率趋势分类任务中均显著优于所有基线模型**。
...
**2. 波动率预测性能**在波动率预测任务中，**NGAT同样稳定地超越了所有基线模型**。
